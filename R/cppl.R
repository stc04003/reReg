...
CPPL.EL <- function(dNit, Yit, Xit, w1t, w2t,
  ...
  est <- EL_saddle(sfun, initial, maxit1, maxit2, tol)
  bhat <- drop(est$beta)
  S <- sfun(bhat)
...
CPPL.GMM <- function(dNit, Yit, Xit, w1t, w2t,
  ...
  est0 <- GMM_NRline(sfun0, initial, diag(p), maxit1, tol)
  ...
  est <- GMM_NRline(sfun, GMM_initial, What, maxit1, tol)
  bhat <- drop(est$beta)
  names(bhat) <- dimnames(Xit)[[2]]
  S <- sfun(bhat)
  Hess <- S$diff.score
...
EL_saddle <- function(score.subject.function, initial,
  ...
  print(paste("iteration", 0, " EL=", L_step, sep = ""))
  for (iter_outer in 1:iteration.max) {
    ...
    if (L_step_new>L_step+tolerance) {
      ...
      T_step <- apply(informationi_step/denominator_step, c(2, 3), sum)
      print(paste("iteration", iter_outer, " EL=", L_step, sep = ""))
    } else
      break
  }
...
GDline <- function(f.diff1, f.diff0 = NULL, initial,
                   iteration.max = 100, tolerance = 1e-7) {
    if (is.function(f.diff0)==FALSE) {
        f.diff0 <- function(theta) {
            value <- f.diff1(theta)$value
            return(value)
        }
    }
    theta_step <- initial
    evaluation_step <- f.diff1(theta_step)
    obj_step <- evaluation_step$value
    grad_step <- evaluation_step$gradient
    print(paste("s", 0, " value=", obj_step, sep = ""))
    for (s in 1:iteration.max) {
        obj_line <- function(alpha) {
            theta_step_new <- theta_step-grad_step * alpha
            objective <- f.diff0(theta_step_new)
            return(objective)
        }
        line_search <- nlminb(start = 0, objective = obj_line)
        if (line_search$objective<obj_step-tolerance) {
            theta_step <- theta_step-grad_step * line_search$par
            evaluation_step <- f.diff1(theta_step)
            obj_step <- evaluation_step$value
            grad_step <- evaluation_step$gradient
            print(paste("s", s, " value=", obj_step, sep = ""))
        } else
            break
    }
