...
A brief introduction of the `Recur()` function is included here. 
...
In the current version (1.1.7), the `reSurv()` function can still be used, but the `reSurv` object will be automatically transform into the corresponding `Recur` object.
For example: 
```{r reSurv2Recur}
...
In the following, we introduced different ways to create an `Resurv` object.
...
```
The function `Recur` prints a list-column `tibble`.
