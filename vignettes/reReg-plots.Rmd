...
Event plots can be produce by plotting the `reSurv` object with `R`'s generic function `plot`, shown in Figure 1.
...
```
The `plotEvents` can also create event plots by groups. 
For example, the following can be used to create event plot by gender:
...
```
Event plot by gender and treatment:
...
is also implemented in the `mcf` function in **`reda`** package [@R:reda].
...
Since CSM plot returns a **`ggplot2`** object, a side-by-side CSM plots, with and without risk set adjustment, is shown in Figure 6.
...
```{r plot-csm-sex-chemo, fig.cap = "Figure 7: CSM plot grouped by `sex`."}
...
The functions `plotEvents` and `plotCSM` can be used to accomdiate recurrent event data with multiple recurrent types.
To illustrate this, we generate hypothetical event types:
```{r multi-events}
library(dplyr)
set.seed(1)
...
```{r multi-event-plot, fig.cap = "Figure 8: Event plots with multiple recurrent event types."}
plotEvents(reSurv(t.stop, id, event2, death) ~ sex, data = readmission)
```

```{r multi-csm, fig.cap = "Figure 9: CSM plot with multiple recurrent event types."}
plotCSM(reSurv(t.stop, id, event2, death) ~ sex, data = readmission)
```


```{r multi-csm2, fig.cap = "Figure 10: CSM plot with multiple recurrent event types."}
