...
**<span style="color:red">The `reSurv()` function is being deprecated in Version 1.1.7.</span>**
A replacement of this function is the `Recur()` function imported from the [`reda`](https://github.com/wenjie2wang/reda) package. 
We give some background on the `reSurv()` function and a brief introduction of the `Recur()` function is included here. 
...
The arguments of `Recur` are as follow
```{r Recur}
args(Recur)
```
...
* `id` specifies the subject identificators. It can be numeric vector, character vector, or a factor vector.  If it is left unspecified, `Recur()` will assume that each row represents a subject.
...
```
For each subject, the function `Recur()` prints intervals to represent the durtion until the next event (a recurrent event or a terminal event).
...
The number of intervals to be printted can be tuned using the `options` and argument `reda.Recur.maxPrint`. 
For example, 
