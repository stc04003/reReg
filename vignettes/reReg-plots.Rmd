...
grid.arrange(p1, p2, ncol=2)
```
As in the `plotEvents`, more complicated 
Figure 8 depicts the CSM plots grouped by `sex` and `chemo`.
```{r plot-csm-sex-chemo, fig.cap = "Figure 8: CSM plot grouped by `sex` and `chemo`."}
...
```
`plotEvents` and `plotCSM` functions can still be applied with `event` replaced with `event2`. 
...
plotCSM(reSurv(t.stop, id, event2, death) ~ sex, adjrisk = FALSE, data = readmission,
        control = list(recurrent.types = c("Type 1", "Type 2", "Type 3")))
```
