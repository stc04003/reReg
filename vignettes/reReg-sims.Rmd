...
```
In this vignette, we demonstrate how to use the `simDat` function in __`reReg`__ package
to simulate recurrent event data from either a Cox-type model, an
accelerated mean model, or a scale-change model. 
The `simDat` function allows the censoring time to be non-informative (independent given covariate)
...
## The `simDat` function {-}
The function `simDat` generates the recurrent times from a recurrent event process, $N(t)$,
with a pre-specified rate function, $\lambda(t)$, and the failure time, $D$,
from a pre-specified hazard function, $h(t)$.
By specifying the argument `type` in the function `simDat`,
...
In `simDat` currently only allows two covariates, i.e., $X = (X_{1}, X_{2})^\top$,
...
The complete list of arguments in `simDat` are as follow
```{r simDat}
library(reReg)
args(simDat)
```
...
## Examples
In the following examples, we simulate recurrent event using `simDat`, with `summary = TRUE`.
```{R set-seed, echo = FALSE}
set.seed(123)
```
Cox-type model:
```{R cox}
dat.cox <- simDat(200, c(-1, 1), c(-1, 1), summary = TRUE)
```
Accelerated mean model:
```{R am}
dat.am <- simDat(200, c(-1, 1), c(-1, 1), type = "am", summary = TRUE)
```
Scale-change model:
```{R sc}
dat.sc <- simDat(200, c(-1, 1), c(-1, 1), type = "sc", summary = TRUE)
```

The output of `simDat` are `tibble` objects.
```{R cox-display}
class(dat.cox)
dat.cox
```
