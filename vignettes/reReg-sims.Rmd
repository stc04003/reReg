...
```
In this vignette, we demonstrate how to use the `simDat` in __`reReg`__ package
to simulate recurrent event data from either a Cox-type model, an
accelerated mean model, or a scale-change model. 
The `simDat` function allows censoring time to be non-informative (independent given covariate)
