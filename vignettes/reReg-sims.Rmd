...
The rate function can be chosen from one of the three models: 

 * Cox-type model:
  $$\lambda(t) = Z\lambda_0(t) e^{X^\top\alpha}.$$
 * Accelerated mean model:
  $$\lambda(t) = Z\lambda_0(te^{X^\top\alpha})e^{X^\top\alpha}.$$
 * Scale-change model:
 ...
 * Cox-type model:
  $$h(t) = Zh_0(t) e^{X^\top\beta}.$$
 * Accelerated mean model:
  $$h(t) = Zh_0(te^{X^\top\beta})e^{X^\top\beta}.$$
 * Scale-change model:
  $$h(t) = Zh_0(te^{X^\top\alpha})e^{X^\top\beta}.$$

The frailty variable $Z$ is used to capture the association between 
the rate function and the hazard function.
...
* `type` a character string specifying the underlying model. The type is distinguished by a vertical bar `|`, with the rate function on the left. For example, setting `type = "cox|am"` generates the recurrent process from a Cox model and the terminal event from an accelerated mean model. Setting `type = "cox"` gives `type = "cox|cox"`.
...
In this example, subject \#1 experienced `r sum(subset(dat.cox, id == 1)$event)` recurrent events 
(at times `r round(subset(dat.cox, id == 1)$Time[1:sum(subset(dat.cox, id == 1)$event)], 3)`)
and died at time `r round(tail(subset(dat.cox, id == 1)$Time, 1), 3)`.
Similarly, subject \#2 experienced `r sum(subset(dat.cox, id == 2)$event)` 
recurrent events (at times `r round(subset(dat.cox, id == 2)$Time[1:sum(subset(dat.cox, id == 2)$event)], 3)`)
and is alive when censored at time `r round(tail(subset(dat.cox, id == 2)$Time, 1), 3)`.
```{R cox-DT}
library(DT)
...
(at times `r round(subset(dat.amcox, id == 1)$Time[1:sum(subset(dat.amcox, id == 1)$event)], 3)`)
and died at time `r round(tail(subset(dat.amcox, id == 1)$Time, 1), 3)`.
Similarly, subject \#2 experienced `r sum(subset(dat.amcox, id == 2)$event)` 
recurrent events (at times `r round(subset(dat.amcox, id == 2)$Time[1:sum(subset(dat.amcox, id == 2)$event)], 3)`)
and is alive when censored at time `r round(tail(subset(dat.amcox, id == 2)$Time, 1), 3)`.
