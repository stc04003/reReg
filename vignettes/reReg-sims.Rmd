...
Let $C_i$ be a noninformative censoring time, which is independent of $N_i(\cdot)$ given $X_i$.
...
```
From accelerated mean model
```{R am}
simDat(200, c(-1, 1), c(-1, 1), type = "am", summary = TRUE)
```
From scale-change model
```{R sc}
simDat(200, c(-1, 1), c(-1, 1), type = "sc", summary = TRUE)
```
