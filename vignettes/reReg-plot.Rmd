...
```{r plotEvents-multiple-events, fig.cap="Figure 5: Event plot with multiple events grouped by `sex`."}
...
The construction of the new data and the stratified event plot in Figure 2d is included in the following
```{r plotEvents-calendar, fig.cap="Figure 6: Event plot on calendar time."}
...
the Nelson-Aalen estimate can be created by applying the generic function 'plot()' to the 'Recur' object with an additional
argument 'mcf = TRUE'.
...
```{r plotMCF-reReg, fig.cap="Figure 8: Creating a MCF plot with `reReg()`"}
...
The arguments are 
  ...
  - `...` is a list of \code{ggplot} objects created by plotting 'reReg' objects.
...
```{r plotMCF-reReg2, fig.cap="Figure 9: Creating a stratified MCF plot with reReg()"}
