...
fit <- reReg(Recur(Time, id, event, status) ~ x1 + x2, data = dat.am, 
             method = "am.GL")
summary(fit)
```
...
fit1 <- reReg(Recur(Time, id, event, status) ~ x1 + x2, data = dat.cox, 
              method = "sc.XCYH")
summary(fit1)
```
Fitting recurrent event data generated from the accelerated model:
```{r sc-XCYH-am, cache = TRUE, tidy = TRUE}
fit2 <- reReg(Recur(Time, id, event, status) ~ x1 + x2, data = dat.am, 
              method = "sc.XCYH")
summary(fit2)
```
