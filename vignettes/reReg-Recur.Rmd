...
The `Recur` objectbundles together a set of recurrent times, failure time, and censoring status, 
...
* `time` a vector that represents the time of recurrent events and censoring, 
...
the \code{Recur()} function performs an internal check for possible issues on the data structure.
The \code{Recur()} function terminates and issues an error message once the check failed if
\code{check = "hard"} (default).
On the contrary, \code{Recur()} would proceed with a warning message when \code{check = "soft"}
or without a warning message when \code{check = "none"}.
...
\item Recurrent event and censoring times cannot be earlier than the orgin time.
\item When the \code{time} argument is a list, the time intervals cannot overlap.
\end{enumerate} 
...
```
The `readmission` example above shows patient id \#1 experienced 2 hostpital readmissions 
