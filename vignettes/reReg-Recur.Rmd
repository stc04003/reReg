...
```
The `readmission` example above shows patient id \#1 experienced 2 hospital readmissions 
...
The slots of the `Recur` S4-class are 
  ...
  - `ord`: indices that sort the response matrix by rows. Soring is in an increasing order by `id`, `time2`, and `-event`.
